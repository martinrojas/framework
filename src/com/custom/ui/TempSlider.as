package com.custom.ui
{
	...
	public class TempSlider extends MovieClip
	{
		//- PRIVATE & PROTECTED VARIABLES -------------------------------------------------------------------------
		 
		        private var _slider:tempSlider;
		...
		/**
		 ...
		          * @return void
		          */       
		public function TempSlider($lowVal:Number = 50, $highVal:Number = 100, $startVal:Number = 72)
		{
			super();
			
			_slider = new tempSlider();
			...
			            this._range =this._highVal - this._lowVal ;
			...
			            if (this._axis == "x")
			            {
				                this._slider.selector[this._axis] = (this._slider.track[this._axis] + (this._percent * this._slider.track[this._changeProp]));
			            }
			            else
			            {
				                this._slider[this._axis] = (this._slider.track[this._axis] - (this._percent * this._slider.track[this._changeProp]));
			            }
			
			
			this._slider.tempLbl.text = (Math.floor(_currentVal)).toString();	           
			            this.initEvents();
		}
		...
		        {
			if (this._slider.hasEventListener(MouseEvent.MOUSE_DOWN)) this._slider.removeEventListener(MouseEvent.MOUSE_DOWN, handleMouseDown);
		...
		        {
			...
			            if (this._axis == "x")
			            {
				                this._slider.selector.startDrag(false, new Rectangle(this._slider.track.x, 86, this._slider.track.width, 0));
			            }
			            
			            this._timer.start();
			           
		...
		        {
			            this._slider.selector.stopDrag();
			            this._timer.reset();
						this.dispatchEvent(new UIEvent(UIEvent.TEMP_STATE_CHANGE, true, (Math.floor(_currentVal))));
			           
		...
		        {
			            this._percent = Math.abs((this._slider.selector[this._axis] - this._slider.track[this._axis]) / this._slider.track[this._changeProp]);
			            this._currentVal = (this._lowVal + (this._range * this._percent));
						this._slider.tempLbl.text = (Math.floor(_currentVal)).toString();
						Controller.logger.log("Slider Current:" + _currentVal +  ", percent :" + (_percent * 100)  +" " + (Math.floor(_currentVal )).toString());
		        }
		 
		//- GETTERS & SETTERS -------------------------------------------------------------------------------------
		   
